...
setwd('C:/CA/CA_model_OCC_220412-v2')
...
#자기 격자에서 옆으로 가는거를 선택을 할 수 있다
# 흐름 방식이 따로 있다

# The units of all values, calculations, functions are based on seconds for time scale and mm for space scale
flowdir <- c("D4") # D8, D4, 4+4N - 관련논문 D4를 많이 사용, 흐름방향 결정
time_step <- c(30) # [seconds] Time step for iteration, 흐름을 계산하는 시간..?
time_interval <- c(450) # [seconds] Time interval in observation data : 초로되어있고 강우관측시간의 간격, 입력자료에 따라 바뀜
...
hydro_data$Rain <- hydro_data$Rain*10 #강우에 10곱해서 넣음 왜? 비가 조금 온 경우는 침수가 제대로 표현안 돼서 강우 크게  
manning_coeff_sur <- c(0.035); #manning 계수 (산이랑 도시 차이 있음-도시:0.03_shao논문 참고고)
LAI <- c(3.76) # Leaf Area Index # 비가 떨어지면 바로 유출되지 않는 부분들 고려
P_max <- 0.935+0.498*LAI+0.00575*LAI^2 # [mm] # 최대 유출량 계산
#침투에 관한 내용들
i_init  <- c(2.30) # [mm/hr]
...
K_Sur=c(0.3);  
Depth_Sur=c(5)*cm_to_mm; # [cm] Soil depth, 토양깊이 사면은 토양층이 잘 유지되니까 100가능하지만 도시는 5로 임의로 작게 설정
E_Sat_Sur_cons_val <- c(60) # Effective saturation must exceed 0, 침투율
